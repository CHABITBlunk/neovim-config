...
--
-- load with `local provider = require "habit.utils.status.provider"`
...
function M.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require "habit.utils.ffi"
...
function M.lsp_progress(opts)
  ...
  return function()
    local _, Lsp = next(habit.lsp.progress) -- TODO fix this line once lsp is set up
...
function M.lsp_client_names(opts)
  ...
  return function(self)
    ...
    for _, client in pairs(vim.lsp.get_active_clients {bufnr = self and self.bufnr or 0 }) do
      if client.name == "null_ls" and opts.expand_null_ls then
